<?php
// Limpieza semanal automática: los lunes se eliminan pedidos cancelados antiguos
if (date('N') == 1) { // 1 = Lunes
    $fechaLimite = date('Y-m-d', strtotime('-1 week'));
    $eliminarAntiguos = ejecutarSQL::consultar("DELETE FROM venta WHERE Estado='cancelado' AND Fecha < '$fechaLimite'");
}
?>

<p class="lead" style="font-size: 20px;">
    Puede ver las ventas cancelados aqui.
</p>
<div class="alert alert-info" style="margin-top: 10px; margin-bottom: 20px; font-size: 16px;">
    <i class="fa fa-info-circle"></i> <strong>Nota:</strong> Las ventas se eliminan automáticamente los días lunes.
</div>
        ...
        <div class="col-xs-12">
            ...
            <div class="panel panel-danger">
                <div class="panel-heading text-center">
                    <h4 style="font-size: 28px;">Ventas Canceladas</h4>
                    <?php if (date('N') == 1): ?>
                        <p class="text-warning" style="margin: 5px 0 0 0; font-size: 16px;"><i class="fa fa-info-circle"></i> <strong>Hoy es lunes:</strong> Se han eliminado las ventas canceladas antiguas.</p>
                    <?php endif; ?>
                </div>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            <?php
                                ...
                                $pedidos=mysqli_query($mysqli,"SELECT SQL_CALC_FOUND_ROWS * FROM venta WHERE Estado='cancelado' LIMIT $inicio, $regpagina");
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <span class="label label-danger" style="font-size: 16px;">Cancelado</span>
                                </td>
                                <td class="text-center">
                                    <!-- Botón de Actualizar agregado -->
                ...
                <div class="text-center">
                  <ul class="pagination">
                    ...
                    <?php else: ?>
                        <li>
                            <a href="configAdmin.php?view=ordercancelled&pag=<?php echo $pagina-1; ?>" style="font-size: 17px;">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        </li>
                    <?php endif; ?>
                    ...
                    <?php
                        for($i=1; $i <= $numeropaginas; $i++ ){
                            if($pagina == $i){
                                echo '<li class="active"><a href="configAdmin.php?view=ordercancelled&pag='.$i.'" style="font-size: 17px;">'.$i.'</a></li>';
                            }else{
                                echo '<li><a href="configAdmin.php?view=ordercancelled&pag='.$i.'" style="font-size: 17px;">'.$i.'</a></li>';
                            }
                        }
                    ?>
...
<!-- Modal para actualizar pedidos cancelados -->
    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '.btn-up-order', function(e){
            ...
            $.ajax({
                ...
                error: function(){
                    $('#OrderSelect').html('<div class="alert alert-danger">Error al cargar los datos de la venta</div>');
                }
            });
        });
